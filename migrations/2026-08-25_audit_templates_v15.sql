-- =========================================================
-- WorkPulse — Audit 2026 templates: v15 (active) + v10 (inactive alternate)
-- 2026-08-25
--
-- Run after 2026-08-25_audit_conditions_schema.sql.
--
-- Seeds two generations of the "Store Hygiene" and "Audit" checklists side
-- by side, as four independent templates in the existing audit_templates /
-- audit_categories / audit_parameters / audit_parameter_options tables —
-- the same tables and the same per-template-id isolation that already lets
-- "Store Hygiene" and "Audit" coexist today. No new tables, no PHP changes:
-- modules/audit.php already reads/writes every template generically by id.
--
--   'Store Hygiene'        / 'Audit'        — v15 sheet, is_active = 1
--   'Store Hygiene (v10)'  / 'Audit (v10)'  — v10 sheet, is_active = 0
--
-- v15 is the default going forward. The v10 pair stays fully usable —
-- flip it on any time from Audit Templates admin (Edit → Active → Save,
-- the checkbox already on that screen) to let auditors pick the old
-- checklist again; no code change needed either way. It is named "(v10)"
-- rather than "(Archived)" because it was never live under the plain
-- name — it is a parked alternate, not retired history.
--
-- Source sheets: audit_templates_Audit_2026_v10.xlsx and
-- audit_templates_Audit_2026_v15.xlsx, "Audit Checklist" tab. Every
-- question, category, weight, condition, action hint and points value
-- below is copied unchanged from the two reference migrations
-- (20260811_audit_templates_v10.sql and
-- 20260825_audit_templates_v15_refresh.sql) — only the template name and
-- is_active flag on the v10 half changed, from the plain/active names
-- those files used (they were written assuming v10 had already been live
-- and needed archiving) to the versioned/inactive ones above, since
-- nothing has actually gone live yet on this database.
--
...
-- across that section's questions in both source sheets.
--
...
-- (option_mode 'radio' throughout both sheets — neither uses 'checkbox').
...
--
-- This is a one-time seed, not idempotent — it always INSERTs four new
-- templates. Re-running this file will create duplicates. Before running,
-- confirm none of the four names above already exist:
--   SELECT name, is_active FROM audit_templates
--    WHERE name IN ('Store Hygiene','Audit','Store Hygiene (v10)','Audit (v10)');
-- =========================================================

START TRANSACTION;

-- =========================================================
-- OLD FORMAT (v10) — inactive by default, admin-toggleable
-- =========================================================

-- =========================================================
-- TEMPLATE: Store Hygiene (v10)   -- old format, inactive by default
-- 5 sections, 30 questions
-- =========================================================
INSERT INTO `audit_templates` (`name`, `is_active`) VALUES ('Store Hygiene (v10)', 0);
...
INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Is the store music on and playing the right way?', 'value', 12.00, 12, 1, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
  (@prm, 'Music is on, with the correct playlist and volume during opening hours', 'Add a short video', 12.00, 1),
...
INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'How did the staff behave with customers?', 'value', 22.00, 22, 2, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
  (@prm, 'Polite and helpful, no complaints', 'Check that no CRM complaint is open', 22.00, 1),
  (@prm, 'Small slip, fixed on the spot', 'Note the coaching you gave', 11.00, 2),
...
INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Are there any suspicious numbers in the reward points data?', 'value', 12.00, 12, 3, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
...
INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Is the stock set as per the planogram and are all price tags visible?', 'value', 30.00, 30, 4, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
  (@prm, 'Set exactly as per planogram, and every tag is there and easy to read', 'Add a photo of the display', 30.00, 1),
  (@prm, 'Small changes or a few tags missing, fixed during the audit or a ticket raised', 'Write a note or check the ticket number', 15.00, 2),
  (@prm, 'Not set as per planogram, or tags missing/wrong with no ticket', 'Add a photo of the display', 0.00, 3);

INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Does the store smell fresh?', 'value', 10.00, 10, 5, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
...
INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Is the ambience set up as per SOP?', 'value', 14.00, 14, 6, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
...
INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Is the staff doing suggestive selling (all offers)?', 'value', 40.00, 40, 1, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
  (@prm, 'Staff is pushing all the offers', 'Check CCTV and upload video of dealings', 40.00, 1),
  (@prm, 'Only some offers are being pushed', 'Note what they missed', 20.00, 2),
  (@prm, 'Not doing it at all', 'Write a note', 0.00, 3);

INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Are Swiggy/Zomato rejections and the visibility score being watched?', 'value', 30.00, 30, 2, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
  (@prm, 'Checked, and there are no rejections that could be avoided', 'Add a screenshot of the dashboard', 30.00, 1),
  (@prm, 'Some avoidable rejections, and a ticket is raised for the extra quantity/cancellation', 'Attach the ticket', 15.00, 2),
  (@prm, 'Not being watched, or lots of avoidable rejections with no ticket', 'Write a note', 0.00, 3);

INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Does the store manager know the targets and sales numbers?', 'value', 15.00, 15, 3, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
...
INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Is the staff aware of all the active offers?', 'value', 15.00, 15, 4, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
...
-- Equipment -- 17 questions, scored out of 100
INSERT INTO `audit_categories` (`template_id`, `name`, `weightage`, `sort_order`)
VALUES (@tpl, 'Equipment', 100, 3);
SET @cat := LAST_INSERT_ID();

INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Is the brand signage working and clean?', 'value', 5.00, 5, 1, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
  (@prm, 'Lit up, working and clean', 'Add a photo', 5.00, 1),
  (@prm, 'Not working, but a ticket is raised', 'Check the ticket number', 2.00, 2),
  (@prm, 'Not working or dirty, and no ticket raised', 'Add a photo', 0.00, 3);

INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Is the CCTV working?', 'value', 12.00, 12, 2, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
  (@prm, 'Working, and recording is available', 'Check the last recording date', 12.00, 1),
...
INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Is the pastry counter working?', 'value', 10.00, 10, 3, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
  (@prm, 'Working, and temperature is as per SOP', 'Add a photo of the temperature display', 10.00, 1),
  (@prm, 'Not working, but a ticket is raised', 'Check the ticket number', 5.00, 2),
  (@prm, 'Not working, and no ticket raised', 'Write a note', 0.00, 3);

INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Is the TV working?', 'value', 10.00, 10, 4, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
  (@prm, 'Working, and image is as per SOP', 'Add a photo of the screen', 10.00, 1),
  (@prm, 'Not working, but a ticket is raised', 'Check the ticket number', 5.00, 2),
  (@prm, 'Not working, and no ticket raised', 'Write a note', 0.00, 3);

INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Is the printer working?', 'value', 6.00, 6, 5, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
  (@prm, 'Working fine', 'No action', 6.00, 1),
  (@prm, 'Not working, but a ticket is raised', 'Check the ticket number', 3.00, 2),
  (@prm, 'Not working, and no ticket raised', 'Write a note', 0.00, 3);

INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Is the computer working?', 'value', 6.00, 6, 6, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
...
INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Is the air curtain working?', 'value', 5.00, 5, 7, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
  ...
  (@prm, 'Not working, but a ticket is raised', 'Check the ticket number', 3.00, 2),
  (@prm, 'Not working, and no ticket raised', 'Write a note', 0.00, 3),
  (@prm, 'Not available at this outlet', 'Note that it\'s not installed', 5.00, 4);

INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Is the puff warmer working?', 'value', 5.00, 5, 8, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
  (@prm, 'Working, and operated as per SOP', 'Add a photo', 5.00, 1),
  (@prm, 'Not working, but a ticket is raised', 'Check the ticket number', 3.00, 2),
  (@prm, 'Not working, and no ticket raised', 'Write a note', 0.00, 3),
  (@prm, 'Not available at this outlet', 'Note that it\'s not installed', 5.00, 4);
...
INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Is the insect killer working?', 'value', 4.00, 4, 9, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
  (@prm, 'Working fine', 'No action', 4.00, 1),
  (@prm, 'Not working, but a ticket is raised', 'Check the ticket number', 2.00, 2),
  (@prm, 'Not working, and no ticket raised', 'Write a note', 0.00, 3);

INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Is the microwave working?', 'value', 4.00, 4, 10, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
  (@prm, 'Working, and operated as per SOP', 'No action', 4.00, 1),
  (@prm, 'Not working, but a ticket is raised', 'Check the ticket number', 2.00, 2),
  (@prm, 'Not working, and no ticket raised', 'Write a note', 0.00, 3);

INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Is the puff warmer clean?', 'value', 4.00, 4, 11, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
  (@prm, 'In use and clean', 'Add a photo (if in use)', 4.00, 1),
  (@prm, 'Not in use', 'Add a photo (if not in use)', 2.00, 2),
  (@prm, 'In use and dirty', 'Add a photo', 0.00, 3);

INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Is the pastry cooler clean?', 'value', 5.00, 5, 12, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
  (@prm, 'Clean', 'Add a photo', 5.00, 1),
  (@prm, 'Not clean', 'Add a photo', 0.00, 2);

INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Is the insect killer clean?', 'value', 4.00, 4, 13, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
  (@prm, 'Clean', 'Add a photo', 4.00, 1),
  (@prm, 'Not clean', 'Add a photo', 0.00, 2);

INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Is the microwave clean?', 'value', 3.00, 3, 14, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
  (@prm, 'Clean', 'Add a photo', 3.00, 1),
  (@prm, 'Not clean', 'Add a photo', 0.00, 2);

INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Is the AC filter cleaned properly?', 'value', 5.00, 5, 15, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
  (@prm, 'Filter cleaned (ladder was available)', 'Add a photo', 5.00, 1),
  (@prm, 'Filter not cleaned', 'Write a note', 0.00, 2);

INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Is preventive maintenance done as per SOP?', 'value', 6.00, 6, 16, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
  (@prm, 'Preventive maintenance done as per SOP', 'Add the maintenance record or a photo', 6.00, 1),
  (@prm, 'Preventive maintenance not done as per SOP', 'Write a note', 0.00, 2);

INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Is the AC set to 24-26 C?', 'value', 6.00, 6, 17, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
  (@prm, 'Set between 24 and 26 C', 'Add a photo of the remote or AC display', 6.00, 1),
  (@prm, 'Set outside the range, but there\'s a good reason', 'Write the reason', 3.00, 2),
  (@prm, 'Set outside the range with no reason', 'Add a photo of the display', 0.00, 3);

-- People -- 2 questions, scored out of 100
INSERT INTO `audit_categories` (`template_id`, `name`, `weightage`, `sort_order`)
VALUES (@tpl, 'People', 100, 4);
SET @cat := LAST_INSERT_ID();

INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Is the staff well-trained, with no training gaps noticed?', 'value', 8.00, 50, 1, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
...
INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Does the staff seem well-motivated, with no concerns noticed?', 'value', 8.00, 50, 2, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
...
-- Operation -- 1 questions, scored out of 100
INSERT INTO `audit_categories` (`template_id`, `name`, `weightage`, `sort_order`)
VALUES (@tpl, 'Operation', 100, 5);
SET @cat := LAST_INSERT_ID();

INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Is the outlet\'s civil infrastructure in good condition (no repair needed)?', 'value', 10.00, 100, 1, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
  (@prm, 'Yes', 'No action', 10.00, 1),
  (@prm, 'No', 'Add a photo and note what needs to be repair', 0.00, 2);

-- =========================================================
-- TEMPLATE: Audit (v10)   -- old format, inactive by default
-- 6 sections, 46 questions
-- =========================================================
INSERT INTO `audit_templates` (`name`, `is_active`) VALUES ('Audit (v10)', 0);
SET @tpl := LAST_INSERT_ID();

-- Cash -- 6 questions, scored out of 100
INSERT INTO `audit_categories` (`template_id`, `name`, `weightage`, `sort_order`)
VALUES (@tpl, 'Cash', 100, 1);
SET @cat := LAST_INSERT_ID();

INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Do the system cash, counter cash and banking tally?', 'value', 32.00, 32, 1, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
  (@prm, 'System cash, counter cash and banking all match (tallied)', 'Attach the deposit slip', 32.00, 1),
  (@prm, 'A difference exists, but a voucher or pending bill from staff is available', 'Attach the voucher or pending bill', 16.00, 2),
  (@prm, 'Cash is short with nothing to explain it', 'Write the short amount', 0.00, 3);

INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Is banking done on time?', 'value', 18.00, 18, 2, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
  (@prm, 'Banking done before the given time', 'Attach the deposit slip', 18.00, 1),
...
INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Are all cash-memo pages in the manual bill book?', 'value', 12.00, 12, 3, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
  (@prm, 'All pages are there and in order', 'No action', 12.00, 1),
  (@prm, 'A page is missing, or the bill book isn\'t available', 'Write the missing page or bill numbers', 0.00, 2);

INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Is the manual bill book kept properly?', 'value', 8.00, 8, 4, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
  (@prm, 'Bill book is correct and up to date', 'No action', 8.00, 1),
  (@prm, 'Bill book is messy or incomplete', 'Write a note', 0.00, 2);

INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Was the expense voucher made and sent to HO within 15 days?', 'value', 18.00, 18, 5, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
  (@prm, 'Made and sent to HO within 15 days', 'No action', 18.00, 1),
  (@prm, 'Not made, or sent late (after 15 days)', 'Write a note', 0.00, 2);

INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Is the GR receipt/voucher file kept in order?', 'value', 12.00, 12, 6, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
  (@prm, 'Receipts and vouchers are filed neatly', 'No action', 12.00, 1),
  (@prm, 'Receipts/vouchers are placed randomly', 'Write a note', 0.00, 2);

-- Equipment -- 17 questions, scored out of 100
INSERT INTO `audit_categories` (`template_id`, `name`, `weightage`, `sort_order`)
VALUES (@tpl, 'Equipment', 100, 2);
SET @cat := LAST_INSERT_ID();
...
INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Procurement handling as per SOP', 'value', 10.00, 10, 2, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
  (@prm, 'Are GI/GR issues sorted within a day? Dispatch/assembly GI/GR cleared within a day', 'No action', 10.00, 1),
  (@prm, 'Are GI/GR issues sorted within a day? Outlet-to-outlet GI/GR still pending after a day', 'Write a note', 0.00, 2);

INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Storage handling as per SOP', 'value', 8.00, 8, 3, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
...
INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Was the finished goods stock count done (anything short)?', 'value', 18.00, 18, 4, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
  (@prm, 'Count done, nothing short', 'No action', 18.00, 1),
  (@prm, 'Count not done, or something is short', 'Write the difference', 0.00, 2);

INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Was the trading goods stock count done (anything short)?', 'value', 18.00, 18, 5, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
...
INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Are there more than 2 alternate items in any category?', 'value', 6.00, 6, 6, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
  (@prm, '2 or fewer alternate items in every category', 'No action', 6.00, 1),
  (@prm, 'More than 2 alternate items in a category', 'Write a note', 0.00, 2);

INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Is the advance order book kept properly?', 'value', 10.00, 10, 7, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
  (@prm, 'Kept properly and up to date', 'No action', 10.00, 1),
...
INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Did the staff wear gloves while handling products?', 'value', 35.00, 42, 1, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
...
INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Is the staff in proper uniform (uniform, belt, grooming)?', 'value', 20.00, 24, 2, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
  (@prm, 'Full uniform, crocs, belt and grooming all good', 'Add a photo', 20.00, 1),
  (@prm, 'Mostly fine, one thing missing', 'Add a photo and note what\'s missing', 10.00, 2),
  (@prm, 'Not in proper uniform', 'Add a photo', 0.00, 3);

INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Are the staff wearing crocs (footwear)?', 'value', 6.00, 7, 3, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
...
INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Is the staff\'s hair neat and well-groomed (shave, hairstyle)?', 'value', 6.00, 7, 4, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
...
INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Are the staff\'s nails properly trimmed?', 'value', 6.00, 7, 5, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
...
INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Are the staff wearing their caps properly?', 'value', 6.00, 7, 6, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
...
INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Is the staff free from any foul smell?', 'value', 5.00, 6, 7, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
...
-- Operation -- 6 questions, scored out of 100
INSERT INTO `audit_categories` (`template_id`, `name`, `weightage`, `sort_order`)
VALUES (@tpl, 'Operation', 100, 5);
SET @cat := LAST_INSERT_ID();
...
INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Is garbage cleared regularly?', 'value', 6.00, 7, 1, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
...
INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Is the customer washroom and wash basin clean and hygienic?', 'value', 18.00, 20, 2, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
...
INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Are the main glass door, glass wall and outside area clean?', 'value', 20.00, 22, 3, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
  (@prm, 'All of it is clean', 'Add a photo', 20.00, 1),
  (@prm, 'One or two spots clean, the rest not', 'Add a photo and note what\'s pending', 10.00, 2),
  (@prm, 'Nothing cleaned', 'Add a photo', 0.00, 3);

INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Are the ceiling, platform and floor clean?', 'value', 16.00, 18, 4, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
  (@prm, 'All of it is clean', 'Add a photo', 16.00, 1),
  (@prm, 'Partly clean', 'Add a photo and note what\'s pending', 8.00, 2),
  (@prm, 'Not clean', 'Add a photo', 0.00, 3);

INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Are the pastry trays clean?', 'value', 14.00, 15, 5, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
  (@prm, 'Clean', 'Add a photo', 14.00, 1),
  (@prm, 'Not clean', 'Add a photo', 0.00, 2);

INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Is the cash counter clean and tidy?', 'value', 16.00, 18, 6, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
...
INSERT INTO `audit_categories` (`template_id`, `name`, `weightage`, `sort_order`)
VALUES (@tpl, 'Compliance', 100, 6);
SET @cat := LAST_INSERT_ID();

INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Is the weighing machine licence in the outlet?', 'value', 30.00, 30, 1, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
  (@prm, 'Valid licence is there', 'Add a photo of the licence', 30.00, 1),
  (@prm, 'Close to expiry, but a renewal ticket is raised', 'Check the ticket number', 15.00, 2),
  (@prm, 'Not there or expired, and no ticket raised', 'Write a note', 0.00, 3);

INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Is the GST certificate in the outlet?', 'value', 35.00, 35, 2, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
  (@prm, 'Valid GST certificate is there', 'Add a photo', 35.00, 1),
  (@prm, 'Close to expiry, but a renewal ticket is raised', 'Check the ticket number', 17.00, 2),
  (@prm, 'Not there or expired, and no ticket raised', 'Write a note', 0.00, 3);

INSERT INTO `audit_parameters`
  (`category_id`, `parameter_text`, `type`, `max_value`, `score_weightage`, `sort_order`, `is_active`)
VALUES (@cat, 'Is the FSSAI licence in the outlet?', 'value', 35.00, 35, 3, 1);
SET @prm := LAST_INSERT_ID();
INSERT INTO `audit_parameter_options`
  (`parameter_id`, `option_text`, `action_hint`, `points`, `sort_order`)
VALUES
