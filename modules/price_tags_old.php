...
function persist(){
  ...
  try{
    localStorage.setItem('pricetags.state.v2', JSON.stringify(out));
    localStorage.setItem('pricetags.items.v2', JSON.stringify(state.items));
  }catch(e){}
}
function restore(){
  try{
    ...
    const i = JSON.parse(localStorage.getItem('pricetags.items.v2') || 'null');
    if(Array.isArray(i) && i.length) state.items = i;
  }catch(e){}
